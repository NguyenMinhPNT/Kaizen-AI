    ...
    return Row(
      children: [
        ...
        _Chip(
          ...
          color:
              habit.currentStreak > 0 ? AppColors.warning : AppColors.textHint,
        ),
        const SizedBox(width: AppDimensions.sm),
        _Chip(
...
class _HabitMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        _menuItem(Icons.play_arrow_outlined, 'Start Timer', 'timer'),
  ...
  void _handleMenuAction(BuildContext context, String action) {
    ...
    switch (action) {
      case 'timer':
        context.push(RoutePaths.timerForHabit(habit.id));
      case 'edit':
        context.push(RoutePaths.editHabit(habit.id));
      case 'levelUp':
        bloc.add(LevelUpHabitEvent(habit.id));
      case 'levelDown':
        bloc.add(LevelDownHabitEvent(habit.id));
      case 'archive':
        bloc.add(ArchiveHabitEvent(habit.id));
      case 'delete':
        bloc.add(DeleteHabitEvent(habit.id));
    }
  }
}
